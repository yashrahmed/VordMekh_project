...
def create_system_prompt_for_chai_customization():
    ...
    Your task is to provide a recipe that is tailored to the number of servings.
...
def create_system_prompt_for_chatbot():
    ...
    SelectedRecipe=Adrak Chai | Number of Servings=Not Selected
    ...
    CMD_SET SelectedRecipe=Masala Chai | Number of Servings=Not Selected
    ...
    6. Never print the form state as is in your output.
    """
    return SystemMessage(system_prompt)
...
def get_recipe():
    try:
        ...
        if num_servings is None or selected_chai_recipe is None:
            return jsonify({'error': 'Missing required fields: num_servings, selected_chai_recipe'}), 400
