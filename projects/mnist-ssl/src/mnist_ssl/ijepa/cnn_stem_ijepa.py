...
This is a separate experiment from :mod:`mnist_ssl.ijepa.custom_ijepa`. Images are
...
neighboring context/target tokens can have overlapping raw-image receptive fields.
    ...
    python -m mnist_ssl.ijepa.cnn_stem_ijepa --epochs 50 --seed 0
...
from mnist_ssl.baselines.mae import (
    DATASET_DIR,
    IMG_SIZE,
    TransformerEncoder,
    make_transform,
...
def find_checkpoint(epochs: int | None = None, n_targets: int = N_TARGETS) -> Path:
    ...
    if epochs is not None:
        ...
        if not path.exists():
            raise FileNotFoundError(
                ...
                f"`python -m mnist_ssl.ijepa.cnn_stem_ijepa --epochs {epochs} --n-targets {n_targets}`."
            )
        return path
    candidates = sorted(
        (
    ...
    if not candidates:
        raise FileNotFoundError(
            ...
            f"`python -m mnist_ssl.ijepa.cnn_stem_ijepa`."
        )
    return candidates[-1]
